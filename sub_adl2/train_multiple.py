...
class DataTrainingArguments:
    ...
    output_file: Optional[str] = field(
        default=None,
        metadata={"help": "An optional input output data file to evaluate the perplexity on (a text file)."},
    )
...
def main():
    ...
    def preprocess_function(examples):
        ...
        paragraph_idx = examples["paragraphs"]
        second_sentences = [
            [context[p] for p in paragraphs] for paragraphs in paragraph_idx
        ]
        ...
        if "relevant" in examples.keys():
            ...
            inputs['label'] = [paragraphs.index(rel) for rel, paragraphs in zip(relevant, paragraph_idx)]
        return inputs
    ...
    # Prediction / write result to a file
    if training_args.do_predict:
        logger.info("*** Predict ***")
        ...
        json.dump(
            output, 
            open(data_args.output_file, 'w',encoding='utf-8'), 
            indent=2, 
            ensure_ascii=False
        )
