...
class DataTrainingArguments:
    ...
    output_file: Optional[str] = field(
        default=None,
        metadata={
            "help": "An optional output data file to predict a test file " "(a jsonlines or csv file)."
        },
    )
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.train_file is not None:
            ...
            # if extension == "jsonl":
            #     with jsonlines.open(data_args.train_file) as dic:
            #         file = open(data_args.train_file[:-1], "w")
            #         json.dump([i for i in dic], file)
            #         data_args.train_file = data_args.train_file[:-1]
        if data_args.validation_file is not None:
            data_files["validation"] = data_args.validation_file
            extension = data_args.validation_file.split(".")[-1]
            # if extension == "jsonl":
            #     with jsonlines.open(data_args.validation_file) as dic:
            #         file = open(data_args.validation_file[:-1], "w")
            #         json.dump([i for i in dic], file)
            #         data_args.validation_file = data_args.validation_file[:-1]
        if data_args.test_file is not None:
            data_files["test"] = data_args.test_file
            extension = data_args.test_file.split(".")[-1]
            # if extension == "jsonl":
            #     with jsonlines.open(data_args.test_file) as dic:
            #         file = open(data_args.test_file[:-1], "w")
            #         json.dump([i for i in dic], file)
            #         data_args.test_file = data_args.test_file[:-1]
        if extension == "jsonl":
            extension = "json"
        raw_datasets = load_dataset(
            extension,
    ...
    # Metric
    # metric = get_rouge # load_metric("rouge")
    ...
    def compute_metrics(eval_preds):
        ...
        result = get_rouge(decoded_preds, decoded_labels, avg=True)
